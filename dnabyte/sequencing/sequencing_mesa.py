...
class MESA(SimulateSequencing):
    ...
    def simulate(self, data):
        ...
        modified_sequences, error_dicts = self.sequencing_simulation(data.data, method_id=self.sequencing_method)
                
        info = {}
        info['eror_dict'] = error_dicts
        info['number_of_sequencing_errors'] = sum([len(d) for d in error_dicts])
    ...
    def sequencing_simulation(self, sequences, method_id):
        ...
        file_path = os.path.join(parent_dir, 'dnabyte', 'ErrorChannels', 'seq_table.json')
