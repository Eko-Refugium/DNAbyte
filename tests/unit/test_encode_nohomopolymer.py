...
from dnabyte.encoding.no_homopolymer.encode import NoHomoPoly
from dnabyte.params import Params

class TestNoHomopolymerEncodingDecoding(unittest.TestCase):
    ...
    def test_encode_decode_nohomopolymer_all_configurations(self):
        """Test NoHomopolymer encoding/decoding with all error correction configurations."""
        ...
        for config in self.test_configs:
            with self.subTest(config=config['name']):
                ...
                # Encode the data
                coder = NoHomoPoly(config['params'], logger=self.logger)
                ...
                # Verify no homopolymers in encoded sequences
                for seq in encoded_data:
                    for i in range(len(seq) - 1):
                        self.assertNotEqual(seq[i], seq[i+1], 
                            f"Homopolymer found at position {i} in sequence for {config['name']}: {seq}")
