...
from dnabyte.encoding.goldman.encode import Goldman
from dnabyte.params import Params


class TestGoldmanEncodingDecoding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Goldman uses Huffman-based ternary encoding with overlapping segments.
        # It does NOT support primers or outer RS codes.
        self.test_configs = [
            {
                ...
                'note': 'Basic Goldman encoding/decoding without error correction'
            },
            {
                'name': 'ltcode_only',
                'params': Params(
    ...
    def test_encode_decode_goldman_all_configurations(self):
        """Test Goldman encoding/decoding with all error correction configurations."""
        ...
        for config in self.test_configs:
            with self.subTest(config=config['name']):
                ...
                # Encode the data
                coder = Goldman(config['params'], logger=self.logger)
